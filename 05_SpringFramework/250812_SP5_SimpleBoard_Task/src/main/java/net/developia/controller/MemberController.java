...
// 회원 관련 웹 요청을 처리하는 Spring MVC 컨트롤러
// 로그인, 로그아웃 등의 기능 담당
@Slf4j
@Controller
...
public class MemberController {
    ...
    // 로그인 페이지 표시
    // Get 요청으로 member/login 경로에 접근 시 호출
    @GetMapping("login")
    public String login() {
        return "member/login";
    }
    ...
    // 사용자 로그인 요청 처리
    // POST 요청으로 /member/login 경로에 접근 시 호출
    /**
    ...
    */
    ...
    public String login(@ModelAttribute MemberDTO memberDTO, HttpSession session, Model model) {
        ...
        try {
            ...
            return "article/result"; // 에러 메시지를 보여줄 뷰 경로
        }
    }

    /**
     * 사용자 로그아웃 요청을 처리합니다.
     * GET 요청으로 /member/logout 경로에 접근 시 호출됩니다.
     *
